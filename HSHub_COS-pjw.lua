...
-- ════════════════════════════════════════════════════════════════════
-- SHRINE COOLDOWN LATCH (time-based: detect -> skip -> idle -> auto-resume)
-- Read from the tablet's own TimerGui ("29m 58s" / "AVAILABLE NOW"). Keyed by
-- offerNameOf so the 3 hardcore "Shadow" altars SHARE one cooldown. The latch lets
-- us keep skipping an on-cooldown shrine even after we leave its region (tablet
-- unloads -> live status reads nil), and auto-RESUME the instant the timer elapses
-- ("lanjut"). Cleared on server/realm hop (fresh server = shrine ready again).
-- ════════════════════════════════════════════════════════════════════
local _shrineCooldownUntil = {}   -- offerName -> tick() when it becomes farmable again
-- MEAT REGIONS (separate from shrines): a region is blacklisted when, after scanning,
-- it has NO meat >= MeatMinValue. Holds until server-hop / leave. regionMeatLog mirrors
-- each region's best meat value for the on-screen log panel.
local _regionEmpty   = {}   -- region (shrine display name) -> true if scanned and empty this server
local regionMeatLog  = {}   -- region -> best meat Value found (0 = none)
...
-- read the tablet's live timer and set/clear the latch (only acts when in-region/loaded)
local function stampShrineCooldown(name)
    local txt = getShrineStatusText(name)        -- nil = tablet not loaded -> leave latch as-is
    if txt == nil then return end
    local key = offerNameOf(name)
    if txt:upper():find('AVAILABLE') then
        _shrineCooldownUntil[key] = nil            -- ready -> resume
    else
        local secs = parseCooldownSecs(txt)
        if secs then _shrineCooldownUntil[key] = tick() + secs end
    end
end
-- true = on cooldown (skip it); false = farmable now OR status unknown (try it)
local function shrineOnCooldown(name)
    local u = _shrineCooldownUntil[offerNameOf(name)]
    if u and (u - tick()) > 0 then return true end  -- latched cooldown not yet elapsed
    return shrineAvailable(name) == false           -- else trust the live read (nil/unknown -> not cd)
end
local function clearShrineCooldowns()
    for k in pairs(_shrineCooldownUntil) do _shrineCooldownUntil[k] = nil end
end
local function clearRegionBlacklist()   -- on hop / leave: every region scannable again
    for k in pairs(_regionEmpty)  do _regionEmpty[k]  = nil end
    for k in pairs(regionMeatLog) do regionMeatLog[k] = nil end
end

-- Return the shrine to farm next.
-- Walks shrineActivationOrder (first toggled ON = highest priority), then falls back
-- to scanning S.ArtifactToggles directly so the AUTONOMOUS orchestrator (which sets a
-- toggle without touching the order queue) is also honored. Skips shrines on cooldown;
-- if every enabled shrine is on cooldown, returns the first one so the caller can show
-- the idle notification and wait (it auto-resumes once a timer resets).
local function getActiveShrine()
    local fallback = nil
    for _, n in ipairs(shrineActivationOrder) do
        if S.ArtifactToggles[n] then
            if not shrineOnCooldown(n) and not _regionEmpty[n] then return n end   -- ready + has meat
            fallback = fallback or n                        -- remember first skipped one
        end
    end
    for n, on in pairs(S.ArtifactToggles) do                -- orchestrator-driven (no order queue)
        if on and n ~= fallback then
            if not shrineOnCooldown(n) and not _regionEmpty[n] then return n end
...
-- (_shrineCooldownUntil + parseCooldownSecs now live above, next to shrineOnCooldown)
...
task.spawn(function()
    while true do
        ...
        for name, lbl in pairs(shrineStatusLabels) do
            pcall(function()
                stampShrineCooldown(name)   -- keep the time-based cooldown latch fresh while in-region
                local txt = getShrineStatusText(name)
                local meat = regionMeatLog[name]
                local meatStr = (meat == nil) and ''
                    or (meat > 0 and (' · meat ' .. meat) or (_regionEmpty[name] and ' · NO meat (skip)' or ''))
                lbl:Set((txt and ('Status: ' .. txt) or 'Status: — (luar region)') .. meatStr)
            end)
        end
...
task.spawn(function()
    while true do
        ...
        if shrineName then pcall(function()
            ...
            -- TP RATE-LIMITER: never teleport more often than every 0.6s (the one hard
            -- pacing rule). Grab + offer fire instantly between TPs.
            local function tpTo(pos)
                local dt = tick() - (S._lastFarmTP or 0)
                if dt < 0.6 then task.wait(0.6 - dt) end
                pcall(function() root.CFrame = CFrame.new(pos) end)
                S._lastFarmTP = tick()
            end

            -- Be IN this shrine's region (that streams its meat). If the tablet isn't
            -- loaded, TP to each known position to stream the region in.
            local tablet = getShrineTablet(shrineName)
            if not tablet then
                for _, known in ipairs(tabletPositions(shrineName)) do
                    tpTo(known + Vector3.new(0, 8, 0))
                    task.wait(0.6)                 -- let the region stream in
                    tablet = getShrineTablet(shrineName)
                    if tablet then break end
                end
                if not tablet then return end      -- unknown region -> orchestrator handles next cycle
            end

            -- Shrine (deposit target) on cooldown -> idle; auto-resumes when the timer resets.
            if shrineOnCooldown(shrineName) then
            ...
            -- Best offer-meat (Carcass, Value >= MeatMinValue) streamed in THIS region.
            local myTier = 0
            pcall(function()
                local d = char:FindFirstChild('Data')
                myTier = (d and tonumber(d:GetAttribute('Tier'))) or 0
            end)
            local function findBestMeat()
                ...
                if foodFolder then
                    for _, m in ipairs(foodFolder:GetChildren()) do
                        if isOfferMeat(m:GetAttribute('FoodDataName'))
                            ...
                            if val >= (S.MeatMinValue or 100) and not (locked and val <= 15) then
                                local part = m:IsA('BasePart') and m
                                    or (m:IsA('Model') and (m.PrimaryPart or m:FindFirstChildWhichIsA('BasePart')))
                                if part and val > bestVal then bestM, bestPart, bestVal, bestLocked = m, part, val, locked end
                            end
                        end
                    end
                end
                return bestM, bestPart, bestVal, bestLocked
            end

            local held = tonumber(char:GetAttribute('HeldCount')) or 0
            if held < 1 then
                local bestM, bestPart, bestVal, bestLocked = findBestMeat()
                if not bestM then task.wait(0.6); bestM, bestPart, bestVal, bestLocked = findBestMeat() end  -- region may still be streaming
                if not bestM then
                    -- NO meat >=100 in this region -> blacklist it (until hop/leave) + log it.
                    _regionEmpty[shrineName] = true
                    regionMeatLog[shrineName] = 0
                    return
                end
                regionMeatLog[shrineName] = bestVal      -- save region's best meat for the panel
                tpTo(bestPart.Position + Vector3.new(0, 4, 0))     -- TP to meat (rate-limited)
                if not bestLocked then
                    local full = getRemote('FoodPickup')
                    if full then pcall(function() full:InvokeServer(bestM) end) end
                    task.wait(0.15)                       -- grab ~instant
                end
            ...
            -- Carrying -> TP to the shrine (deposit target) and offer.
            if held >= 1 then
                S._farmProgressAt = tick()
                tpTo(tablet.Position + Vector3.new(0, 6, 0))       -- TP to shrine (rate-limited)
                local before = tonumber(char:GetAttribute('HeldCount')) or held
                local wo = getRemote('WardenOffering')
                if wo then pcall(function() wo:InvokeServer(offerNameOf(shrineName)) end) end
                task.wait(0.2)                            -- offer ~instant
                pcall(function() stampShrineCooldown(shrineName) end)
                local after = tonumber(char:GetAttribute('HeldCount')) or before
                if after < before then
                    local key = offerNameOf(shrineName)
                    if not _shrineCooldownUntil[key] then _shrineCooldownUntil[key] = tick() + 60 end
                end
            end
        end) end
...
do
    ...
    S.LobbyBtnX, S.LobbyBtnY = 775, 150     -- in-game "return to lobby" button (fallback; has ~10s countdown)
    S.RefViewX, S.RefViewY = 0, 0           -- reference viewport the coords were captured on (set at UI build / editable)
    ...
    -- ═══ device scaling for MANUAL coords (realm menu / realm targets / lobby) ═══
    -- Manual taps are raw pixels captured on ONE device. To land on the same UI on ANY
    -- device, scale by (live viewport / reference viewport). Reference = the viewport the
    -- coords were captured on (S.RefViewX/Y, set at UI build to this device, editable).
    -- ratio 1 (same device, or ref unset) = coords pass through unchanged (no regression).
    local function curViewport()
        local cam = workspace.CurrentCamera
        return (cam and cam.ViewportSize) or Vector2.new(0, 0)
    end
    local function scaledTap(x, y)
        local vp = curViewport()
        local rx, ry = tonumber(S.RefViewX) or 0, tonumber(S.RefViewY) or 0
        if rx > 0 and ry > 0 and vp.X > 0 and vp.Y > 0 then
            x = x * (vp.X / rx); y = y * (vp.Y / ry)
        end
        vimTap(math.floor(x + 0.5), math.floor(y + 0.5))
    end
    ...
    local function serverHop()
        ...
        clearShrineCooldowns()   -- fresh server -> shrine timers reset
        clearRegionBlacklist()   -- fresh server -> every region scannable again
        pcall(function()
            local ok, raw = pcall(function()
                return game:HttpGet('https://games.roblox.com/v1/games/' .. tostring(game.PlaceId)
                    .. '/servers/Public?sortOrder=Asc&limit=100')
    ...
    -- at the lobby iff the SaveSelectionGui Play/Restart buttons exist (lobby-only).
    local function atLobby() return findPlayButton() ~= nil or findRestartButton() ~= nil end
    -- Return to lobby, then DETECT arrival before doing anything else (robust on any
    -- device/ping -- not a blind sleep). Try the instant DespawnRemote first (no countdown);
    -- if that doesn't land us in the lobby within a few seconds, tap the in-game Lobby
    -- button (S.LobbyBtnX/Y, scaled) and wait out its ~10s countdown (15s safety cap).
    local function returnToLobby()
        if atLobby() then return true end
        pcall(function() invoke('DespawnRemote') end)                 -- instant path (no countdown)
        local t = tick()
        while not atLobby() and tick() - t < 4 do task.wait(0.4) end  -- grace for the remote
        if not atLobby() and not (S.LobbyBtnX == 0 and S.LobbyBtnY == 0) then
            statusSet('despawn slow -> tap Lobby button (wait countdown)')
            panelHide(); task.wait(0.5); scaledTap(S.LobbyBtnX, S.LobbyBtnY); task.wait(0.2); panelShow()
        end
        t = tick()
        while not atLobby() and tick() - t < 15 do task.wait(0.5) end -- detect lobby (covers the 10s countdown)
        return atLobby()
    end

    -- realm switch via AUTO-CLICK (manual-typed coords, viewport-scaled = any device).
    -- TeleportService(placeId) is REJECTED by the game, so we return to the lobby then tap
    -- the in-lobby Realms UI like a real player: return-to-lobby -> open menu -> target realm.
    local function realmSwitch(tx, ty)
        if tx == 0 and ty == 0 then statusSet('realm coord not set'); return false end
        clearShrineCooldowns()                       -- new realm/server -> shrine timers reset
        clearRegionBlacklist()                       -- new realm/server -> regions scannable again
        statusSet('realm switch -> return to lobby')
        if not returnToLobby() then statusSet('realm switch: lobby not reached (timeout)'); panelShow(); return false end
        panelHide(); task.wait(0.6)
        if not (S.RealmMenuX == 0 and S.RealmMenuY == 0) then scaledTap(S.RealmMenuX, S.RealmMenuY); task.wait(1.5) end
        scaledTap(tx, ty); task.wait(0.3); panelShow()
        return true
    end

    -- ═══ ORCHESTRATOR ═══
    -- (region blacklist now lives in _regionEmpty, set by the farm loop on an empty scan)
            ...
            if mode and (not busy) then
                ...
                pcall(function()
                    ...
                    if atLobby then
                        ...
                        spawnFor(mode)
                        -- (region blacklist + cooldowns persist in-server across respawns;
                        --  they reset only on server-hop / realm-switch)
                    else
                        ...
                        local active, skipped = nil, nil
                        local cdCount, emptyCount = 0, 0
                        for _, n in ipairs(order) do
                            if shrineOnCooldown(n) then
                                cdCount = cdCount + 1               -- deposit target on cooldown -> skip
                            elseif _regionEmpty[n] then
                                emptyCount = emptyCount + 1         -- region scanned, no meat >=100 (this server)
                            elseif shrineReachable(n) then
                                if not active then active = n end   -- first reachable region with a usable shrine
                            else
                                skipped = skipped or n              -- shrine pos unknown -> try next
                            end
                        end
                        -- REALM ROTATION (cooldown-based, live each cycle): normal -> when the 3
                        -- priority shrines are all on cooldown, rotate to HARDCORE (Shadow);
                        -- hardcore -> when Shadow is on cooldown, rotate back to NORMAL.
                        if S.RealmHopArtifact and (tick() - lastRealmHop > 60) then
                            if (not IS_HARDCORE) and shrineOnCooldown('Ardor') and shrineOnCooldown('Novus') and shrineOnCooldown('Eigion') then
                                lastRealmHop = tick(); statusSet('3 priority cooldown -> rotate to HARDCORE')
                                realmSwitch(S.RealmHardcoreX, S.RealmHardcoreY); return
                            elseif IS_HARDCORE and (shrineOnCooldown('Shadow Up') or shrineOnCooldown('Shadow Middle') or shrineOnCooldown('Shadow Down')) then
                                lastRealmHop = tick(); statusSet('shadow cooldown -> rotate to NORMAL')
                                realmSwitch(S.RealmNormalX, S.RealmNormalY); return
                            end
                        end
                        for n in pairs(S.ArtifactToggles) do S.ArtifactToggles[n] = (n == active) end
                        if active then
                            statusSet(('farming %s  (cd %d · empty %d)'):format(active, cdCount, emptyCount))
                        elseif emptyCount > 0 and (tick() - lastHop > 30) then
                            -- nothing farmable AND at least one region ran out of meat >=100 -> hop
                            lastHop = tick(); statusSet('all regions out of meat -> server hop')
                            task.wait(1.0); serverHop()
                        elseif skipped then
                            statusSet(skipped .. ' pos unknown - run ShrineHunter')
                        elseif cdCount > 0 and S.AutoFarmHopWhenDone and (tick() - lastHop > 30) then
                            lastHop = tick(); statusSet('shrines on cooldown -> server hop')
                            task.wait(1.0); serverHop()
                        else
                            statusSet('idle (cd ' .. cdCount .. ')')
                        end
                    end
                end)
                busy = false
    ...
    Sec:AddToggle({ Name = 'Realm Hop (normal 3 -> hardcore Shadow -> back)', Key = 'RealmHopArtifact', Default = false,
        Tip = 'Normal: when Ardor+Novus+Eigion all on cooldown -> return to lobby (instant; detects arrival) -> open Realms menu -> tap HARDCORE. Hardcore: when Shadow on cooldown -> back to NORMAL. Coords below auto-scale to any device via RefView.',
        Callback = function(v) S.RealmHopArtifact = v end })
    Sec:AddLabel('Realm-switch button coords. Menu=0 to skip. Coords auto-scale to any device (see Reference viewport).', Color3.fromRGB(150, 150, 180))
    ...
    Sec:AddTextbox({ Name = 'Lobby button X (in-game)', Default = '775', Callback = function(v) S.LobbyBtnX = tonumber(v) or 0 end })
    Sec:AddTextbox({ Name = 'Lobby button Y (in-game)', Default = '150', Callback = function(v) S.LobbyBtnY = tonumber(v) or 0 end })
    -- ── reference viewport (all-device coord scaling) ──
    do
        local cam = workspace.CurrentCamera
        local vp  = (cam and cam.ViewportSize) or Vector2.new(0, 0)
        S.RefViewX, S.RefViewY = math.floor(vp.X), math.floor(vp.Y)   -- default ref = THIS device (coords exact here)
        Sec:AddLabel('Coords captured on YOUR device. RefView = its viewport; taps scale by (live / RefView) elsewhere.', Color3.fromRGB(150, 150, 180))
        local vpLbl = Sec:AddLabel(('Viewport sekarang: %d x %d'):format(math.floor(vp.X), math.floor(vp.Y)), Color3.fromRGB(180, 220, 255))
        task.spawn(function() while true do task.wait(3); pcall(function()
            local c = workspace.CurrentCamera; local v = c and c.ViewportSize
            if v then vpLbl:Set(('Viewport sekarang: %d x %d  (RefView %d x %d)')
                :format(math.floor(v.X), math.floor(v.Y), math.floor(tonumber(S.RefViewX) or 0), math.floor(tonumber(S.RefViewY) or 0))) end
        end) end end)
        Sec:AddTextbox({ Name = 'RefView X (capture-device width)',  Default = tostring(math.floor(vp.X)), Callback = function(v) S.RefViewX = tonumber(v) or 0 end })
        Sec:AddTextbox({ Name = 'RefView Y (capture-device height)', Default = tostring(math.floor(vp.Y)), Callback = function(v) S.RefViewY = tonumber(v) or 0 end })
    end
